...
class SybaseEventGenerator(bspump.Generator):
    def __init__(self, app, pipeline, id=None, config=None):
        ...
        self.Username = asab.Config.get("sybase", "username")
        self.Password = asab.Config.get("sybase", "password")
        self.Host = asab.Config.get("sybase", "host")
        self.Port = asab.Config.get("sybase", "port")
        self.Driver = asab.Config.get("sybase", "driver")
        self.Database = asab.Config.get("sybase", "database")
        self.Server = asab.Config.get("sybase", "server")

        self.QueryLocation = asab.Config.get("sybase", "query")

        self.resolution = asab.Config.get("sybase", "resolution")

        self.daily = asab.Config.get("sybase", "daily")
        ...
        self.connection_string = "Driver={};UID={};PWD={};Server={};DBN={};CommLinks=TCPIP{};DriverUnicodeType=1".format(
            self.Driver,
            self.Username,
            self.Password,
            self.Server,
            self.Database,
            "{{host={};port={}}}".format(self.Host, self.Port),
        )
    ...
    async def generate(self, context, event, depth):
        ...
        try:
            ...
            L.log(
                asab.LOG_NOTICE,
                "SybaseEventGenerator error: {} {}".format(traceback.format_exc(), e),
            )
    ...
    async def _generate(self, context, event, depth):
        try:
            ...
            L.log(
                asab.LOG_NOTICE,
                "resolution in config must be either an expression or an number {}".format(
                    e
                ),
            )
        ...
        try:
            ...
            L.log(
                asab.LOG_NOTICE,
                "Incorrect Daily format. Please use 0 for False 1 for True {}".format(
                    e
                ),
            )
        if self.daily:
        ...
        L.log(
            asab.LOG_NOTICE,
            "Connection took {} seconds, Query took {} seconds".format(
                elapsed_connection, elapsed_time
            ),
        )
        ...
        L.log(
            asab.LOG_NOTICE,
            "Total number of events {} for previous timestamp {}".format(
                event_count, self.previous_timestamp
            ),
        )
